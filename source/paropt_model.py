...
from source.paropt_data_loader import ParoptDataset
from source.paropt_visualize import visualize
...
class ParoptModel(PocoModel):
    ...
    def __init__(self, output_names, in_channels, out_channels, k,
        ...
        super().__init__(output_names, in_channels, out_channels, k,
                 lambda_l1, debug, in_file, results_dir, padding_factor, name, network_latent_size,
                 gen_subsample_manifold_iter, gen_subsample_manifold, gen_resolution_global,
                 rec_batch_size, gen_refine_iter, workers)

        self.network = ParoptNetwork(in_channels=self.in_channels, latent_size=self.network_latent_size,
    ...
    #             ground_truth=gt.squeeze(),ParoptDataset predicted=pred_labels.squeeze(), prediction_name=None)
    #         return eval_dict

    #     eval_dict = compare_classification(pred=pred, gt=gt_data['occ'])
    #     eval_dict['abs_dist_rms'] = np.nanpointnet.pytorch/utils/visualize.py
    ...
    def test_step(self, batch, batch_idx):
        ...
        pred = ParoptDataset.un_norm(pred)
        pred = {
            "cgDepth": round(pred[0]),
            "confidence": False,
            "depth": round(pred[1]),
            "fullDepth": round(pred[2]),
            "iters": round(pred[3]),
            "pointWeight": pred[4],
            "preClean": False,
            "samplesPerNode": pred[5],
            "scale": pred[6],
            "visibleLayer": False,
            "id": batch["id"]
        }
        ...
        results = {'shape_id': batch["id"], 'loss': loss}
        return results

    def on_test_epoch_end(self):
        visualize(self.test_step_outputs)
    ...
    #     if batch['pts_ms'].shape[0] > 1:mean
...
class ParoptNetwork(PocoNetwork):

    def __init__(self, in_channels, latent_size, out_channels, k):
        super().__init__(in_channels, latent_size, out_channels, k)
